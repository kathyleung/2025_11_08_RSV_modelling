function out = fig_period_rsv_by_4age_groups_rates(mcRecMonthlyByAgePrctile, periodCaseRSV,  populationAgeGroup, dateZero, cutoffCOVID)
    ...
    fig_handle = figure(11); % Use a different figure number
    clf; % Clear figure before plotting
    
    ylimArr = [10,1,1,6];

    % --- Loop through the 4 age groups --- 
    populationAgeGroupMeanRev = zeros(1,4);
    for ageGroupIdx = 1:4
        ...
        if ageGroupIdx == 1
            ...
            populationAgeGroupMeanRev(ageGroupIdx) = sum(populationAgeGroupMean(originalAgeGroups));
        else
            % Other age groups map directly (with offset)
            originalAgeGroups = ageGroupIdx + 2;
            populationAgeGroupMeanRev(ageGroupIdx) = populationAgeGroupMean(originalAgeGroups);
        end
        ...
        yearly_mean_model_median = yearly_mean_model_median/populationAgeGroupMeanRev(ageGroupIdx)*100000;
        yearly_mean_model_lowerCI = yearly_mean_model_lowerCI/populationAgeGroupMeanRev(ageGroupIdx)*100000;
        yearly_mean_model_upperCI = yearly_mean_model_upperCI/populationAgeGroupMeanRev(ageGroupIdx)*100000;
        yearly_mean_observed = yearly_mean_observed/populationAgeGroupMeanRev(ageGroupIdx)*100000;
        ...
        currentMaxY = ylimArr(ageGroupIdx);
        ...
        if mod(ageGroupIdx, 2) == 1
             ylabel(ax, 'Yearly number of cases per 100,000', 'FontSize', 10);
        end
