function out = fig_serology_HK_BJ_Thailand(...
...
%   out - Figure handle
    ...
    uniquePMIDs = uniquePMIDs(3);  % Select specific Asian studies
    
    % Create figure
    fig_handle = figure(7);
    
    % First subplot: HK local data
    subplot(3, 1, 1);
    unique_ages = (1:length(seroprevDataHKRec.age_start))-1;
    
    % Create left y-axis for model predictions (blue)
    yyaxis left;
    
    % Plot model results - blue filled dots with error bars (shifted slightly left)
    errorbar(unique_ages - 0.05, mcRecSerologyHKPrctile(:,1), ...
    ...
    errorbar(unique_ages + 0.05, dataSerologyHKMean, ...
    ...
    % Add title
    title(sprintf('%s, %s', 'Hong Kong', '2020'), 'FontSize', 10);
    ...
    xticks_labels = {'0-11m','12-23m','2-4y','5-9y','10-14y','15-19y','20-39y','40-59y','60-80y'};
    ...
    xlim([-0.5, max(unique_ages)*1.05]);

    % Third subplot: Thailand data
    subplot(3, 1, 3);
    % Thailand
    ...
    sgtitle('Model fit to RSV seroprevalence by age, country and year', 'FontSize', 10, 'FontWeight', 'bold');
