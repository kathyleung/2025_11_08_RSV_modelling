 ...
 *
 * _coder_age_SEIR_RSV_api.c
 *
 * Code generation for function '_coder_age_SEIR_RSV_api'
 *
 */
...
static creal_T dc_emlrt_marshallIn(const emlrtStack *sp, const mxArray *src,
                                   const emlrtMsgIdentifier *msgId);
...
static real_T (*gc_emlrt_marshallIn(const emlrtStack *sp, const mxArray *src,
                                    const emlrtMsgIdentifier *msgId))[200750];
...
static real_T (*pb_emlrt_marshallIn(const emlrtStack *sp,
                                    const mxArray *nullptr,
                                    const char_T *identifier))[200750];

static void q_emlrt_marshallIn(const emlrtStack *sp, const mxArray *u,
                               const emlrtMsgIdentifier *parentId);

static real_T (
    *qb_emlrt_marshallIn(const emlrtStack *sp, const mxArray *u,
                         const emlrtMsgIdentifier *parentId))[200750];
...
static boolean_T xb_emlrt_marshallIn(const emlrtStack *sp, const mxArray *src,
                                     const emlrtMsgIdentifier *msgId);
                    ...
                    const emlrtMsgIdentifier *parentId)
{
  ...
  y.length = d_emlrt_marshallIn(sp, emlrtAlias(propValues[0]), &thisId);
...
static const mxArray *emlrt_marshallOut(const struct1_T *u)
{
  ...
  int32_T i;
  y = NULL;
  emlrtAssign(&y, emlrtCreateStructMatrix(1, 1, 27, (const char_T **)&b_sv[0]));
  b_y = NULL;
  b_iv[0] = 1;
  ...
  c_y = NULL;
  m = emlrtCreateDoubleScalar(u->stateVmAb);
  emlrtAssign(&c_y, m);
  emlrtSetFieldR2017b(y, 0, "stateVmAb", c_y, 25);
  emlrtSetFieldR2017b(y, 0, "stateVvax", c_emlrt_marshallOut(u->stateVvax), 26);
  return y;
}
...
static real_T (*gc_emlrt_marshallIn(const emlrtStack *sp, const mxArray *src,
                                    const emlrtMsgIdentifier *msgId))[200750]
{
  static const int32_T dims[2] = {8030, 25};
  real_T(*ret)[200750];
  int32_T b_iv[2];
  boolean_T bv[2] = {false, false};
  emlrtCheckVsBuiltInR2012b((emlrtConstCTX)sp, msgId, src, "double", false, 2U,
                            (const void *)&dims[0], &bv[0], &b_iv[0]);
  ret = (real_T(*)[200750])emlrtMxGetData(src);
  emlrtDestroyArray(&src);
  return ret;
}
                   ...
                   const emlrtMsgIdentifier *parentId)
{
  ...
  thisId.fIdentifier = "length";
  y.length = d_emlrt_marshallIn(sp, emlrtAlias(propValues), &thisId);
...
static void r_emlrt_marshallIn(const emlrtStack *sp, const mxArray *u,
                               const emlrtMsgIdentifier *parentId)
{
  wb_emlrt_marshallIn(sp, emlrtAlias(u), parentId);
  emlrtDestroyArray(&u);
}
...
void age_SEIR_RSV_api(age_SEIR_RSVStackData *SD, const mxArray *const prhs[56],
                      const mxArray **plhs)
{
  ...
  static const uint32_T uv[12] = {913576686U,  128563055U,  542890784U,
                                  ...
                                  3461043304U, 2874626807U, 929727076U,
                                  1867049519U, 898646805U,  2968773611U};
  ...
  real_T(*relTransmissionPHSMList)[200750];
  ...
  emlrtCheckArrayChecksumR2018b(&st, prhs[27], false, &b_iv[0],
                                (const char_T **)&b_sv[0], &uv[0]);
  emlrtCheckArrayChecksumR2018b(&st, prhs[28], false, &b_iv[0],
  ...
  probVmAb = c_emlrt_marshallIn(&st, emlrtAliasP(prhs[22]), "probVmAb");
  probVvax = emlrt_marshallIn(&st, emlrtAlias(prhs[23]), "probVvax");
  totalPopulation =
      e_emlrt_marshallIn(&st, emlrtAlias(prhs[24]), "totalPopulation");
  ageGroupDefRangeAll =
      g_emlrt_marshallIn(&st, emlrtAlias(prhs[25]), "ageGroupDefRangeAll");
  muBirthRate = i_emlrt_marshallIn(&st, emlrtAlias(prhs[26]), "muBirthRate");
  k_emlrt_marshallIn(&st, emlrtAliasP(prhs[27]), "populationSizeTable",
                     &populationSizeTable);
  ab_emlrt_marshallIn(&st, emlrtAliasP(prhs[28]), "populationSizeTableRev",
                      &populationSizeTableRev);
  phyContactMatr =
      hb_emlrt_marshallIn(&st, emlrtAlias(prhs[29]), "phyContactMatr");
  conversationContactMatr =
      hb_emlrt_marshallIn(&st, emlrtAlias(prhs[30]), "conversationContactMatr");
  iniInfectiousProp =
      c_emlrt_marshallIn(&st, emlrtAliasP(prhs[31]), "iniInfectiousProp");
  iniRecoveredProp =
      c_emlrt_marshallIn(&st, emlrtAliasP(prhs[32]), "iniRecoveredProp");
  a__1 = jb_emlrt_marshallIn(&st, emlrtAliasP(prhs[33]), "~1");
  dateStart = c_emlrt_marshallIn(&st, emlrtAliasP(prhs[34]), "dateStart");
  numDays = c_emlrt_marshallIn(&st, emlrtAliasP(prhs[35]), "numDays");
  numTimeSteps = c_emlrt_marshallIn(&st, emlrtAliasP(prhs[36]), "numTimeSteps");
  dt = c_emlrt_marshallIn(&st, emlrtAliasP(prhs[37]), "dt");
  xiMaternalImmunity =
      c_emlrt_marshallIn(&st, emlrtAliasP(prhs[38]), "xiMaternalImmunity");
  omegaInfectionImmunity =
      c_emlrt_marshallIn(&st, emlrtAliasP(prhs[39]), "omegaInfectionImmunity");
  sigmaExposure =
      c_emlrt_marshallIn(&st, emlrtAliasP(prhs[40]), "sigmaExposure");
  gammaZeroPrimaryInfection = c_emlrt_marshallIn(&st, emlrtAliasP(prhs[41]),
                                                 "gammaZeroPrimaryInfection");
  g1ReductionInfectiousDuration = c_emlrt_marshallIn(
      &st, emlrtAliasP(prhs[42]), "g1ReductionInfectiousDuration");
  g2ReductionInfectiousDuration = c_emlrt_marshallIn(
      &st, emlrtAliasP(prhs[43]), "g2ReductionInfectiousDuration");
  delta1RelSuscep =
      c_emlrt_marshallIn(&st, emlrtAliasP(prhs[44]), "delta1RelSuscep");
  delta2RelSuscep =
      c_emlrt_marshallIn(&st, emlrtAliasP(prhs[45]), "delta2RelSuscep");
  delta3RelSuscep =
      c_emlrt_marshallIn(&st, emlrtAliasP(prhs[46]), "delta3RelSuscep");
  pAsym = nb_emlrt_marshallIn(&st, emlrtAlias(prhs[47]), "pAsym");
  alphaReductionInfectiousness = c_emlrt_marshallIn(
      &st, emlrtAliasP(prhs[48]), "alphaReductionInfectiousness");
  qTransmissPhysical =
      c_emlrt_marshallIn(&st, emlrtAliasP(prhs[49]), "qTransmissPhysical");
  qTransmissConversation =
      c_emlrt_marshallIn(&st, emlrtAliasP(prhs[50]), "qTransmissConversation");
  b1AmpTransmissPeak =
      c_emlrt_marshallIn(&st, emlrtAliasP(prhs[51]), "b1AmpTransmissPeak");
  phiSeasonalShift =
      c_emlrt_marshallIn(&st, emlrtAliasP(prhs[52]), "phiSeasonalShift");
  psiSeasonalWavelength =
      c_emlrt_marshallIn(&st, emlrtAliasP(prhs[53]), "psiSeasonalWavelength");
  probSeropos = c_emlrt_marshallIn(&st, emlrtAliasP(prhs[54]), "probSeropos");
  relTransmissionPHSMList =
      pb_emlrt_marshallIn(&st, emlrtAlias(prhs[55]), "relTransmissionPHSMList");
  /* Invoke the target function */
  emxInitStruct_struct1_T(&st, &SD->f0.out, &j_emlrtRTEI);
  age_SEIR_RSV(&st, *stateM, *stateS0, *stateS1, *stateS2, *stateS3, *stateE0,
               *stateE1, *stateE2, *stateE3, *stateI0, *stateI1, *stateI2,
               *stateI3, *stateA0, *stateA1, *stateA2, *stateA3, *stateR0,
               *stateR1, *stateR2, *stateR3, *stateZ, probVmAb, *probVvax,
               *totalPopulation, *ageGroupDefRangeAll, *muBirthRate,
               &populationSizeTable, &populationSizeTableRev, *phyContactMatr,
               *conversationContactMatr, iniInfectiousProp, iniRecoveredProp,
               &a__1, dateStart, numDays, numTimeSteps, dt, xiMaternalImmunity,
               ...
               qTransmissConversation, b1AmpTransmissPeak, phiSeasonalShift,
               psiSeasonalWavelength, probSeropos, *relTransmissionPHSMList,
               &SD->f0.out);
...
/* End of code generation (_coder_age_SEIR_RSV_api.c) */
