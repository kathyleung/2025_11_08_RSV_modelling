...
% PHSM
relTransmissionPHSM = [...
    1.57E+00	4.41E-01	2.02E-01	2.84E-02	4.34,...
    1.57E+00	4.41E-01	2.02E-01	2.84E-02	4.34,...
    1.57E+00	4.41E-01	2.02E-01	2.84E-02	4.34,...
    1.57E+00	4.41E-01	2.02E-01	2.84E-02	4.34,...
    0.2];
...
if strcmp(countryText,'Hong Kong')
    % Define age groups
    ageGroupDefRange = ageGroupDefRangeUse;
    totalPop = sum(populationSize.population_count);
    ageDistributionFiveyear =  loadAgeDistrCensus(countryText);
    [contactMatrSkin,~] = polymodHKcontactMatrix(ageGroupDefRange,ageDistributionFiveyear,totalPop,SKINCONTACT);
    [contactMatrConversation,ageDistributionOneyear] = polymodHKcontactMatrix(ageGroupDefRange,ageDistributionFiveyear,totalPop,CONVERSATIONCONTACT);
    totalPopulationTemp = calcTotalPopulationArrOneYear(totalPop,ageDistributionOneyear,ageGroupDefRange);
    totalPopulation = (sum(populationSize.population_count)*totalPopulationTemp)/sum(totalPopulationTemp);
end
...
arrayPHSMCutoff = [
    ...
    days(datetime('2021-01-01','Format','uuuu-MM-dd')-dateZero),...
    days(datetime('2022-01-01','Format','uuuu-MM-dd')-dateZero),...
    days(datetime('2023-01-01','Format','uuuu-MM-dd')-dateZero),...
    days(datetime('2024-01-01','Format','uuuu-MM-dd')-dateZero)];
...
indicesMonthlyPHSM =  indicesMonthlyRSV([2,3,4,end]);
...
reportRateAfterPandemicList = repmat([ones([arrayPHSMCutoff(1)/dt,1]);relTransmissionPHSM(end)*ones([(numDays-arrayPHSMCutoff(1))/dt,1])],1,numAgeGroups);
