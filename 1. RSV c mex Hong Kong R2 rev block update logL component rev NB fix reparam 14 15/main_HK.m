...
probSeropos = 0.75;
% Linear overdispersion parameter for weekly lab counts.
% Start close to Poisson so Hong Kong only moves away from it if supported
% by the data.
linDispWeeklyLab = 1e-3;
...
x0 = [omegaInfectionImmunity, sigmaExposure, gammaZeroPrimaryInfection,...
    ...
    expReport, epsilonReport17,epsilonReport18,epsilonReport19,probSeropos,linDispWeeklyLab];
...
savedStateVersion = 'linear_dispersion_nb_weekly_v10_reparam1415_cov';
...
elseif exist(savedMlePath,'file') == 2
    savedMle = readmatrix(savedMlePath);
    if numel(savedMle) == expectedNumPars
        x0 = reshape(savedMle,1,[]);
...
elseif exist(savedMcmcPath,'file') == 2 || exist(savedMlePath,'file') == 2
    disp('Ignoring saved Hong Kong state because it is incompatible with the current sampler version.');
end
