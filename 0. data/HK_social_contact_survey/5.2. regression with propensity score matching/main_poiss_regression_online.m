...
ageGroupDefRange = [-1,10,20,40,65,101];
...
regrTable.age_or_mid_range = findAgeGroupSmallInterv(regrTable.age_or_mid_range,ageGroupDefRange);
regrAge = zeros(numObservations,max(regrTable.age_or_mid_range)-1);
for iiAge = 2:max(regrTable.age_or_mid_range)
    regrAge(regrTable.age_or_mid_range==iiAge,iiAge-1) = 1;
end
ageWeights = AgeDistribution(regrTable.age_or_mid_range)';
% 2. gender: ref male
...
% descriptive
for iiEduIdx = 1:3
    display([length(regrTable.num_diary_contact(regrTable.edu==iiEduIdx & regrTable.mode_survey==2)),...
        mean(regrTable.num_diary_contact(regrTable.edu==iiEduIdx & regrTable.mode_survey==2)),...
        std(regrTable.num_diary_contact(regrTable.edu==iiEduIdx & regrTable.mode_survey==2))]);
end
regrTable.edu(regrTable.edu>6) = 1;
...
% descriptive
for iiIncomeIdx = 1:5
    display([length(regrTable.num_diary_contact(regrTable.income_mid_range==iiIncomeIdx & regrTable.mode_survey==2)),...
        mean(regrTable.num_diary_contact(regrTable.income_mid_range==iiIncomeIdx & regrTable.mode_survey==2)),...
        std(regrTable.num_diary_contact(regrTable.income_mid_range==iiIncomeIdx & regrTable.mode_survey==2))]);
end
...
betaZero = 0;
betaVec = zeros(1,...
    max(regrTable.age_or_mid_range)-1+...
...
regrX = regrX(regrTable.mode_survey==2,:);
regrY = regrY(regrTable.mode_survey==2,:);
% MLE
...
% MCMC
mcSteps = 20000;
if exist('mcmc_result/parameter_step.csv','file') == 2
    stepSize = csvread('mcmc_result/parameter_step.csv');
else
