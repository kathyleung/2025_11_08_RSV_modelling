function out = fig_monthly_rsv_by_age_groups_rates(mcRecMonthlyByAgePrctile, monthlyCaseRSV, populationAgeGroup, dateZero, cutoffCOVID)
    ...
    mcRecMonthlyByAgePrctile = mcRecMonthlyByAgePrctile./repmat(mean(populationAgeGroup),mtrSize(1),1,mtrSize(3))*100000;
    
    % Cut off data at COVID
    monthlyCaseRSV = monthlyCaseRSV(monthlyCaseRSV.month_start<cutoffCOVID,:);
    mtrSize = size(monthlyCaseRSV);
    monthlyCaseRSV{:,4:9} = monthlyCaseRSV{:,4:9}./repmat(mean(populationAgeGroup),mtrSize(1),1)*100000;
    ...
    % figConfig = gcf;
    % set(figConfig,'Position',[0,0,21,29.7],'Units','centimeters')

    % Y lim
    ylimArr = [1500,800,300,10,40,250];
    ...
    for ageGroup = 1:6
        ...
        hModel = plot(monthlyCaseRSV.month_start, mcRecMonthlyByAgePrctile(1:size(monthlyCaseRSV,1),ageGroup,1), '-', ...
            'Color', [0, 0.4470, 0.7410], 'LineWidth', 1);
        
        % Format left y-axis (model predictions)
        ylabel('Model estimate (per 100,000)', 'Color', [0, 0.4470, 0.7410], 'FontSize', 10);
        ax_left = gca;
        ...
        hObs = plot(monthlyCaseRSV.month_start, smooth(monthlyCaseRSV{:,data_col},1), '-', ...
        ...
        for i = 1:length(years)
            ...
            if jan_pos >= min(monthlyCaseRSV.month_start) && jan_pos <= max(monthlyCaseRSV.month_start)+366
        ...
        % Set x-axis limits
        xlim([min(monthlyCaseRSV.month_start), max(monthlyCaseRSV.month_start)+31]);

        if ageGroup == 2
            hLegend = legend([hModel,hObs],{'Model estimates','Observed RSV cases'});
            set(hLegend,'Box','off')
        end
    end
    
    % Add a main title for the whole figure
    sgtitle('Monthly RSV cases per 100,000 by age groups', 'FontSize', 10, 'FontWeight', 'bold');

    AddLetters2Plots(fig_handle, {'a', 'b', 'c','d','e','f'})
