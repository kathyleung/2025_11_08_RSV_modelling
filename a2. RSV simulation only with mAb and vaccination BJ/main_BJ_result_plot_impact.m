...
% 2. MCMC parameters
maxIter = 1000;
mcmcRes = readmatrix('../4. RSV c mex Beijing R1 block update logL component/mcmc_result/China_subnational_Beijing_mcmc_res.csv');
...
% numUsedAgeGroup = length(indicesMonthlyRSV);
...
% yearlyAgeSpecificOutcomeNoIntervention = zeros(length(dateStartArr),numAgeGroup,revNumIterations,6);
% yearlyAgeSpecificOutcomeWithIntervention = zeros(length(dateStartArr),numAgeGroup,revNumIterations,6);
% 
% % Intervention
% for kkStrategy = 1:6
%     if kkStrategy == 1
...
%         yearlyAgeSpecificOutcomeIterNoIntervention = zeros(length(dateStartArr),numAgeGroup);
%         yearlyAgeSpecificOutcomeIterWithIntervention = zeros(length(dateStartArr),numAgeGroup);
...
%             yearlyAgeSpecificOutcomeIterNoIntervention(jjYear,:) = sum(iterOutput.dailyAgeSpecificIncidenceReport_no_intervention_rec(dateRangeRow,:),1);
%             yearlyAgeSpecificOutcomeIterWithIntervention(jjYear,:) = sum(iterOutput.dailyAgeSpecificIncidenceReport_with_intervention_rec(dateRangeRow,:),1);
%             % Calculate sum over date range and normalize by population
...
%         yearlyAgeSpecificOutcomeNoIntervention(:,:,iiIter,kkStrategy) = yearlyAgeSpecificOutcomeIterNoIntervention;
%         yearlyAgeSpecificOutcomeWithIntervention(:,:,iiIter,kkStrategy) = yearlyAgeSpecificOutcomeIterWithIntervention;
%     end
% end
% 
% % Save the key simulation result variables
% save('RSV_simulation.mat', ...
%     'yearlyAgeSpecificIncNoIntervention', 'yearlyAgeSpecificIncWithIntervention',...
%     'yearlyAgeSpecificReportOutcome','yearlyAgeSpecificReportOutcomeRate',...
%     'yearlyAgeSpecificOutcomeNoIntervention','yearlyAgeSpecificOutcomeWithIntervention');

load('RSV_simulation.mat')

% Averted
yearlyAvgReportOutcome = [];
for kkStrategy = 1:6
    % 0-11 months
    temp1 = mean(yearlyAgeSpecificReportOutcomeRate(:,1:12,:,kkStrategy),1);
    temp1Reshp = reshape(temp1,12,300);
    yearlyAvgReportOutcome(1,:,kkStrategy) = prctile(...
        totalPopulation(1:12)*temp1Reshp/sum(totalPopulation(1:12))*10^5,[50,2.5,97.5]); 
    % 60-74 yrs
    temp2_1 = 0.5*mean(yearlyAgeSpecificReportOutcomeRate(:,23,:,kkStrategy),1);
    temp2_2 = mean(yearlyAgeSpecificReportOutcomeRate(:,24,:,kkStrategy),1);
    temp2Reshp = reshape(temp2_1+temp2_2,1,300);
    yearlyAvgReportOutcome(2,:,kkStrategy) = prctile(...
        (0.5*totalPopulation(23)+totalPopulation(24))*temp2Reshp/(0.5*totalPopulation(23)+totalPopulation(24))*10^5,[50,2.5,97.5]);
    % 75+ yrs
    temp3 = mean(yearlyAgeSpecificReportOutcomeRate(:,25,:,kkStrategy),1);
    temp3Reshp = reshape(temp3,1,300);
    yearlyAvgReportOutcome(3,:,kkStrategy) = prctile(...
        totalPopulation(25)*temp2Reshp/totalPopulation(25)*10^5,[50,2.5,97.5]);
    % Total
    temp4 = mean(yearlyAgeSpecificReportOutcomeRate(:,:,:,kkStrategy),1);
    temp4Reshp = reshape(temp4,length(totalPopulation),300);
    yearlyAvgReportOutcome(4,:,kkStrategy) = prctile(...
        totalPopulation*temp4Reshp/sum(totalPopulation)*10^5,[50,2.5,97.5]);
end

% Percentage reduction
percReduction =  yearlyAgeSpecificReportOutcome./yearlyAgeSpecificOutcomeNoIntervention;
for kkStrategy = 1:6
    % 0-11 months
    temp1 = mean(percReduction(:,1:12,:,kkStrategy),1);
    temp1Reshp = reshape(temp1,12,numRnd);
    tempYearlyAgeSpecificOutcomeNoIntervention = reshape(mean(yearlyAgeSpecificOutcomeNoIntervention(:,1:12,:,kkStrategy),1),12,numRnd);
    yearlyAvgReportOutcomePerc(1,:,kkStrategy) = prctile(...
        sum(tempYearlyAgeSpecificOutcomeNoIntervention.*temp1Reshp,1)./sum(tempYearlyAgeSpecificOutcomeNoIntervention,1),[50,2.5,97.5]); 
     % 60-74 yrs
    temp2_1 = 0.5*mean(percReduction(:,23,:,kkStrategy),1);
    temp2_2 = mean(percReduction(:,24,:,kkStrategy),1);
    temp2Reshp = reshape((temp2_1+temp2_2)/1.5,1,numRnd);
    tempYearlyAgeSpecificOutcomeNoIntervention = reshape(mean(...
        yearlyAgeSpecificOutcomeNoIntervention(:,23,:,kkStrategy)+...
        yearlyAgeSpecificOutcomeNoIntervention(:,24,:,kkStrategy),1),1,numRnd);
    yearlyAvgReportOutcomePerc(2,:,kkStrategy) = prctile(...
        sum(tempYearlyAgeSpecificOutcomeNoIntervention.*temp2Reshp,1)./sum(tempYearlyAgeSpecificOutcomeNoIntervention,1),[50,2.5,97.5]); 
    % 75+ yrs
    temp3 = mean(percReduction(:,25,:,kkStrategy),1);
    temp3Reshp = reshape(temp3,1,numRnd);
    tempYearlyAgeSpecificOutcomeNoIntervention = reshape(mean(yearlyAgeSpecificOutcomeNoIntervention(:,25,:,kkStrategy),1),1,numRnd);
    yearlyAvgReportOutcomePerc(3,:,kkStrategy) = prctile(...
         sum(tempYearlyAgeSpecificOutcomeNoIntervention.*temp3Reshp,1)./sum(tempYearlyAgeSpecificOutcomeNoIntervention,1),[50,2.5,97.5]); 
    % Total
    temp4 = mean(percReduction(:,:,:,kkStrategy),1);
    temp4Reshp = reshape(temp4,numAgeGroups,numRnd);
    tempYearlyAgeSpecificOutcomeNoIntervention = reshape(mean(yearlyAgeSpecificOutcomeNoIntervention(:,:,:,kkStrategy),1),[],numRnd);
    yearlyAvgReportOutcomePerc(4,:,kkStrategy) = prctile(...
        sum(tempYearlyAgeSpecificOutcomeNoIntervention.*temp4Reshp,1)./sum(tempYearlyAgeSpecificOutcomeNoIntervention,1),[50,2.5,97.5]);
end

% Percentage reduction for 8 age groups
% Age group mapping to model indices (25 groups):
%   1. 0-11 months:  indices 1:12 (full)
%   2. 12-23 months: index 13 (full)
%   3. 2-4 years:    indices 14:16 (full)
%   4. 5-17 years:   indices 17:18 (full) + index 19 (frac 3/10, ages 15-18 of 15-25)
%   5. 18-59 years:  index 19 (frac 7/10, ages 18-25 of 15-25) + indices 20:22 (full) + index 23 (frac 5/10, ages 55-60 of 55-65)
%   6. 60-74 years:  index 23 (frac 5/10, ages 60-65 of 55-65) + index 24 (full)
%   7. 75+ years:    index 25 (full)
%   8. All ages:     indices 1:25 (full)
yearlyAvgReportOutcomePerc8 = zeros(8,3,6);
for kkStrategy = 1:6
    % 0-11 months (indices 1:12, all full)
    temp1 = mean(percReduction(:,1:12,:,kkStrategy),1);
    temp1Reshp = reshape(temp1,12,numRnd);
    tempYearlyAgeSpecificOutcomeNoIntervention = reshape(mean(yearlyAgeSpecificOutcomeNoIntervention(:,1:12,:,kkStrategy),1),12,numRnd);
    yearlyAvgReportOutcomePerc8(1,:,kkStrategy) = prctile(...
        sum(tempYearlyAgeSpecificOutcomeNoIntervention.*temp1Reshp,1)./sum(tempYearlyAgeSpecificOutcomeNoIntervention,1),[50,2.5,97.5]);
    % 12-23 months (index 13, single full)
    temp2 = mean(percReduction(:,13,:,kkStrategy),1);
    temp2Reshp = reshape(temp2,1,numRnd);
    tempYearlyAgeSpecificOutcomeNoIntervention = reshape(mean(yearlyAgeSpecificOutcomeNoIntervention(:,13,:,kkStrategy),1),1,numRnd);
    yearlyAvgReportOutcomePerc8(2,:,kkStrategy) = prctile(...
        sum(tempYearlyAgeSpecificOutcomeNoIntervention.*temp2Reshp,1)./sum(tempYearlyAgeSpecificOutcomeNoIntervention,1),[50,2.5,97.5]);
    % 2-4 years (indices 14:16, all full)
    temp3 = mean(percReduction(:,14:16,:,kkStrategy),1);
    temp3Reshp = reshape(temp3,3,numRnd);
    tempYearlyAgeSpecificOutcomeNoIntervention = reshape(mean(yearlyAgeSpecificOutcomeNoIntervention(:,14:16,:,kkStrategy),1),3,numRnd);
    ...
    temp4Reshp = reshape((temp4_1+temp4_2)/2.3,1,numRnd);
    tempYearlyAgeSpecificOutcomeNoIntervention = reshape(mean(...
        yearlyAgeSpecificOutcomeNoIntervention(:,17,:,kkStrategy)+...
        yearlyAgeSpecificOutcomeNoIntervention(:,18,:,kkStrategy)+...
        0.3*yearlyAgeSpecificOutcomeNoIntervention(:,19,:,kkStrategy),1),1,numRnd);
    ...
    temp5Reshp = reshape((temp5_1+temp5_2+temp5_3)/4.2,1,numRnd);
    tempYearlyAgeSpecificOutcomeNoIntervention = reshape(mean(...
        0.7*yearlyAgeSpecificOutcomeNoIntervention(:,19,:,kkStrategy)+...
        yearlyAgeSpecificOutcomeNoIntervention(:,20,:,kkStrategy)+...
        yearlyAgeSpecificOutcomeNoIntervention(:,21,:,kkStrategy)+...
        yearlyAgeSpecificOutcomeNoIntervention(:,22,:,kkStrategy)+...
        0.5*yearlyAgeSpecificOutcomeNoIntervention(:,23,:,kkStrategy),1),1,numRnd);
    yearlyAvgReportOutcomePerc8(5,:,kkStrategy) = prctile(...
        sum(tempYearlyAgeSpecificOutcomeNoIntervention.*temp5Reshp,1)./sum(tempYearlyAgeSpecificOutcomeNoIntervention,1),[50,2.5,97.5]);
    ...
    temp6Reshp = reshape((temp6_1+temp6_2)/1.5,1,numRnd);
    tempYearlyAgeSpecificOutcomeNoIntervention = reshape(mean(...
        yearlyAgeSpecificOutcomeNoIntervention(:,23,:,kkStrategy)+...
        yearlyAgeSpecificOutcomeNoIntervention(:,24,:,kkStrategy),1),1,numRnd);
    yearlyAvgReportOutcomePerc8(6,:,kkStrategy) = prctile(...
        sum(tempYearlyAgeSpecificOutcomeNoIntervention.*temp6Reshp,1)./sum(tempYearlyAgeSpecificOutcomeNoIntervention,1),[50,2.5,97.5]);
    % 75+ years (index 25, single full)
    temp7 = mean(percReduction(:,25,:,kkStrategy),1);
    temp7Reshp = reshape(temp7,1,numRnd);
    tempYearlyAgeSpecificOutcomeNoIntervention = reshape(mean(yearlyAgeSpecificOutcomeNoIntervention(:,25,:,kkStrategy),1),1,numRnd);
    yearlyAvgReportOutcomePerc8(7,:,kkStrategy) = prctile(...
        sum(tempYearlyAgeSpecificOutcomeNoIntervention.*temp7Reshp,1)./sum(tempYearlyAgeSpecificOutcomeNoIntervention,1),[50,2.5,97.5]);
    % All ages (indices 1:25, all full)
    temp8 = mean(percReduction(:,:,:,kkStrategy),1);
    temp8Reshp = reshape(temp8,numAgeGroups,numRnd);
    tempYearlyAgeSpecificOutcomeNoIntervention = reshape(mean(yearlyAgeSpecificOutcomeNoIntervention(:,:,:,kkStrategy),1),[],numRnd);
    yearlyAvgReportOutcomePerc8(8,:,kkStrategy) = prctile(...
        sum(tempYearlyAgeSpecificOutcomeNoIntervention.*temp8Reshp,1)./sum(tempYearlyAgeSpecificOutcomeNoIntervention,1),[50,2.5,97.5]);
end

% Plot
fig_reduction_by_age_group_errorbar(yearlyAvgReportOutcomePerc8);