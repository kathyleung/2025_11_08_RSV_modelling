...
% Conversational-contact transmissibility is fixed inside mcmcRes_RSV and
% is not estimated in x0.
...
% Reporting-parameter placeholders retained for consistency with the
% fitted model parameterization.
expReport = [-5.6, -0.533];
...
probSeropos = 0.75;
% Linear dispersion parameter for monthly RSV case counts (Var = mu*(1+linDisp))
linDispMonthlyReport = 20;
...
% 2. MCMC parameters
maxIter = 1000;
mcmcRes = readmatrix('../7. RSV c mex Thailand R2 block update logL component NB fix reparam 14 15/mcmc_result/thailand_mcmc_res.csv');

mcmcRes = mcmcRes(mcmcRes(:,1)~=0,:);
burnInStartIdx = max(1,floor(size(mcmcRes,1)*0.2)+1);
...
mcLinDispMonthlyReport = zeros(revNumIterations,1);
populationAgeGroup = zeros(revNumIterations,size(test_output.populationAgeGroup,2));
...
for iiIter = 1:revNumIterations
    ...
    mcLinDispMonthlyReport(iiIter) = iterOutput.linDispMonthlyReport;
...
save('RSV_simulation_raw_results.mat',...
    ...
    'mcEpsilonByAge','mcLinDispMonthlyReport','yearlyAgeSpecificIAR','populationAgeGroup',...
...
% 2. Monthly record by age — per-age-group NB posterior predictive
mcRecMonthlyByAgeRnd = zeros(length(mcRecMonthlyByAge(:,1,1)),length(mcRecMonthlyByAge(1,:,1)),numRnd);
numAgeGroupsMonthly = length(mcRecMonthlyByAge(1,:,1));
for iiMonth = 1:length(mcRecMonthlyByAge(:,1,1))
    tempMuByAgeAll = reshape(mcRecMonthlyByAge(iiMonth,:,:),[numAgeGroupsMonthly,min(maxIter,revNumIterations)]);
    drawIdx = randsample(1:size(tempMuByAgeAll,2),numRnd,true);
    tempMuByAge = (tempMuByAgeAll(:,drawIdx))';  % numRnd x numAgeGroups
    tempDisp = max(reshape(mcLinDispMonthlyReport(drawIdx),[],1),1e-12);  % numRnd x 1
    % Per-age-group NB: Var(Y) = mu*(1+linDisp), r = mu/linDisp, p = r/(r+mu)
    tempCounts = zeros(numRnd,numAgeGroupsMonthly);
    for iiAge = 1:numAgeGroupsMonthly
        muAge = tempMuByAge(:,iiAge);
        posIdx = muAge > 0;
        if any(posIdx)
            rAge = muAge(posIdx)./tempDisp(posIdx);
            pAge = rAge./(rAge + muAge(posIdx));
            tempCounts(posIdx,iiAge) = nbinrnd(rAge,pAge);
        end
    end
    mcRecMonthlyByAgeRnd(iiMonth,:,:) = reshape(tempCounts',[1,numAgeGroupsMonthly,numRnd]);
end
