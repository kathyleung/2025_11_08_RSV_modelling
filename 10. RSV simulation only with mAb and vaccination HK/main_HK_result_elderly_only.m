...
% 2. MCMC parameters
maxIter = 1000;
mcmcRes = readmatrix('../1. RSV c mex Hong Kong R1 rev block update logL component/mcmc_result/hongkong_mcmc_res.csv');
...
for kkStrategy = 1:6
    if kkStrategy == 1
        ...
        if kkStrategy == 2
            ...
            oVaxCoverageIter = zeros([numAgeGroup,1]);
            oVaxEffectivenessIter = zeros([numAgeGroup,1]);
            oVaxCoverageIter((end-2):end,1) = oVaxCoverage(randsample(numRnd,1),:)./[31.4/2,31.4,39.7]*50;
            oVaxEffectivenessIter((end-2):end,1) = 1;
        else 
            if kkStrategy == 3
                % Strategy 3: oVax only - 31.4% and 39.7%
                mAbCoverageIter = zeros([numAgeGroup,1]);
                mAbEffectivenessIter = zeros([numAgeGroup,1]);
                mVaxCoverageIter = zeros([numAgeGroup,1]);
                mVaxEffectivenessIter = zeros([numAgeGroup,1]);
                ...
                if kkStrategy == 4
                    ...
                    oVaxCoverageIter = zeros([numAgeGroup,1]);
                    oVaxEffectivenessIter = zeros([numAgeGroup,1]);
                    oVaxCoverageIter((end-2):end,1) = oVaxCoverage(randsample(numRnd,1),:)./[31.4/2,31.4,39.7]*75;
                    oVaxEffectivenessIter((end-2):end,1) = 1;
                else
                    if kkStrategy == 5
                        % Strategy 5: oVax only - 95%
                        mAbCoverageIter = zeros([numAgeGroup,1]);
                        mAbEffectivenessIter = zeros([numAgeGroup,1]);
                        mVaxCoverageIter = zeros([numAgeGroup,1]);
                        mVaxEffectivenessIter = zeros([numAgeGroup,1]);
                        oVaxCoverageIter = zeros([numAgeGroup,1]);
                        oVaxEffectivenessIter = zeros([numAgeGroup,1]);
                        oVaxCoverageIter((end-2):end,1) = oVaxCoverage(randsample(numRnd,1),:)./[31.4/2,31.4,39.7]*95;
                        oVaxEffectivenessIter((end-2):end,1) = 1;
                    else 
                        if kkStrategy == 6
                            % Strategy 6: oVax only - 15%
                            mAbCoverageIter = zeros([numAgeGroup,1]);
                            mAbEffectivenessIter = zeros([numAgeGroup,1]);
                            mVaxCoverageIter = zeros([numAgeGroup,1]);
                            mVaxEffectivenessIter = zeros([numAgeGroup,1]);
                            oVaxCoverageIter = zeros([numAgeGroup,1]);
                            oVaxEffectivenessIter = zeros([numAgeGroup,1]);
                            oVaxCoverageIter((end-2):end,1) = oVaxCoverage(randsample(numRnd,1),:)./[31.4/2,31.4,39.7]*15;
                            oVaxEffectivenessIter((end-2):end,1) = 1;
                        end
                    end
                end
...
save('RSV_simulation_elderly.mat', ...
...
load('RSV_simulation_elderly.mat')
