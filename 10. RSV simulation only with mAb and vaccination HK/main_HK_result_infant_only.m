...
for kkStrategy = 1:6
    if kkStrategy == 1
        ...
        if kkStrategy == 2
            ...
            if kkStrategy == 3
                % Strategy 3: mVax only - 50%
                mAbCoverageIter = zeros([numAgeGroup,1]);
                mAbEffectivenessIter = zeros([numAgeGroup,1]);
                mVaxCoverageIter = zeros([numAgeGroup,1]);
                mVaxEffectivenessIter = zeros([numAgeGroup,1]);
                mVaxCoverageIter(1:6,1) = randsample(0.5/0.385*mVaxCoverage,1);
                mVaxEffectivenessIter(1:6,1) = 1;
                oVaxCoverageIter = zeros([numAgeGroup,1]);
                oVaxEffectivenessIter = zeros([numAgeGroup,1]);
            else
                if kkStrategy == 4
                    % Strategy 4: mVax only - 75%
                    mAbCoverageIter = zeros([numAgeGroup,1]);
                    mAbEffectivenessIter = zeros([numAgeGroup,1]);
                    mVaxCoverageIter = zeros([numAgeGroup,1]);
                    mVaxEffectivenessIter = zeros([numAgeGroup,1]);
                    mVaxCoverageIter(1:6,1) = randsample(0.75/0.385*mVaxCoverage,1);
                    mVaxEffectivenessIter(1:6,1) = 1;
                    oVaxCoverageIter = zeros([numAgeGroup,1]);
                    oVaxEffectivenessIter = zeros([numAgeGroup,1]);
                else
                    if kkStrategy == 5
                        % Strategy 5: mVax only - 95%
                        mAbCoverageIter = zeros([numAgeGroup,1]);
                        mAbEffectivenessIter = zeros([numAgeGroup,1]);
                        mVaxCoverageIter = zeros([numAgeGroup,1]);
                        mVaxEffectivenessIter = zeros([numAgeGroup,1]);
                        mVaxCoverageIter(1:6,1) = randsample(0.95/0.385*mVaxCoverage,1);
                        mVaxEffectivenessIter(1:6,1) = 1;
                        oVaxCoverageIter = zeros([numAgeGroup,1]);
                        oVaxEffectivenessIter = zeros([numAgeGroup,1]);
                    else 
                        if kkStrategy == 6
                            % Strategy 6: mVax only - 15%
                            mAbCoverageIter = zeros([numAgeGroup,1]);
                            mAbEffectivenessIter = zeros([numAgeGroup,1]);
                            mVaxCoverageIter = zeros([numAgeGroup,1]);
                            mVaxEffectivenessIter = zeros([numAgeGroup,1]);
                            mVaxCoverageIter(1:6,1) = randsample(0.15/0.385*mVaxCoverage,1);
                            mVaxEffectivenessIter(1:6,1) = 1;
                            oVaxCoverageIter = zeros([numAgeGroup,1]);
                            oVaxEffectivenessIter = zeros([numAgeGroup,1]);
                        end
...
save('RSV_simulation_infant.mat', ...
...
load('RSV_simulation_infant.mat')
