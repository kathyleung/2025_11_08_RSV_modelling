function out = fig_monthly_rsv_by_age_groups_rates(mcRecMonthlyByAgePrctile, monthlyILICaseRSV,populationAgeGroup, dateZero, cutoffCOVID)
    ...
    mcRecMonthlyByAgePrctile = mcRecMonthlyByAgePrctile./repmat(mean(populationAgeGroup,1),mtrSize(1),1,3)*100000;

    % Cut off data at COVID
    monthlyILICaseRSV = monthlyILICaseRSV(monthlyILICaseRSV.month_start<cutoffCOVID,:);
    monthlyILICaseRSV{:,4:8} = monthlyILICaseRSV{:,4:8}./repmat(mean(populationAgeGroup,1),length(monthlyILICaseRSV{:,1}),1)*100000;
    
    % Define age group labels
    age_labels = {'0-4 years', '5-19 years', '20-49 years','50-64 years', '\geq 65 years'};
    ...
    for ageGroup = 1:5
        ...
        x_fill = [monthlyILICaseRSV.month_start; flipud(monthlyILICaseRSV.month_start)];
        y_fill = [mcRecMonthlyByAgePrctile(1:size(monthlyILICaseRSV,1),ageGroup,2); flipud(mcRecMonthlyByAgePrctile(1:size(monthlyILICaseRSV,1),ageGroup,3))];
        fill(x_fill, y_fill, [0, 0.4470, 0.7410], 'FaceAlpha', 0.3, 'EdgeColor', 'none');
        
        % Plot model median prediction (blue line)
        plot(monthlyILICaseRSV.month_start, mcRecMonthlyByAgePrctile(1:size(monthlyILICaseRSV,1),ageGroup,1), '-', ...
            'Color', [0, 0.4470, 0.7410], 'LineWidth', 1);
        
        % Format left y-axis (model predictions)
        ylabel('Model (per 100,000)', 'Color', [0, 0.4470, 0.7410], 'FontSize', 10);
        ax_left = gca;
        ...
        plot(monthlyILICaseRSV.month_start, smooth(monthlyILICaseRSV{:,data_col},1), '-', ...
            'Color', [0.8500, 0.3250, 0.0980], 'LineWidth', 1);
        
        % Format right y-axis (observed data)
        ylabel('Observed (per 100,000)', 'Color', [0.8500, 0.3250, 0.0980], 'FontSize', 10);
        ax_right = gca;
        ...
        years = unique(year(dateZero + monthlyILICaseRSV.month_start));
        ...
        for i = 1:length(years)
            ...
            if jan_pos >= min(monthlyILICaseRSV.month_start) && jan_pos <= max(monthlyILICaseRSV.month_start)
        ...
        % Set x-axis limits
        xlim([min(monthlyILICaseRSV.month_start), max(monthlyILICaseRSV.month_start)]);
    end
    
    % Add a main title for the whole figure
    sgtitle('Monthly RSV ALRI hospitalisations by age groups', 'FontSize', 10, 'FontWeight', 'bold');
    AddLetters2Plots(fig_handle, {'a', 'b', 'c','d','e'})
