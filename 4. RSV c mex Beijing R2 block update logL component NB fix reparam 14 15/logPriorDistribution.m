function out = logPriorDistribution(params)
    % Parameter order follows x0 in main_Beijing.m and unpacking in
    % logLikelihood_RSV.m:
    ...
    % 3. sigma: D_exposure = 1/sigma ~ Gamma(7.111, 0.563)
    sigmaExposure = params(2);
    if sigmaExposure <= 0
        log_p = -1e9;
    else
        D_exposure = 1 / sigmaExposure;
        log_p = log(gampdf(D_exposure, 7.111, 0.563));
        if isinf(log_p) && log_p < 0
            log_p = -1e9; 
        else
    ...
    % 4. gamma0: D_primary = 1/gamma0 ~ Weibull(shape=4.137, scale=8.303)
    gammaZeroPrimaryInfection = params(3);
    if gammaZeroPrimaryInfection <= 0
        log_p = -1e9;
    else
        D_primary = 1 / gammaZeroPrimaryInfection;
        log_p = log(wblpdf(D_primary, 8.303, 4.137)); % scale, shape in MATLAB
        if isinf(log_p) && log_p < 0
            log_p = -1e9;
        else
    ...
    % so implausibly large seasonal amplification is discouraged unless the
    ...
    % together with bounds set in main_Beijing.m.
    ...
    % Parameter 28. Linear overdispersion parameter for monthly counts
    % under Var(Y) = mu * (1 + linDispMonthlyLab).
    % Use a truncated exponential prior to keep Beijing close to Poisson
