...
% Remove overlapping aggregate periods so plots and posterior summaries use
% the same non-duplicated Beijing age data as the fitting script.
...
% 2. MCMC parameters
maxIter = 1000;
mcmcRes = readmatrix('../4. RSV c mex Beijing R2 block update logL component NB fix reparam 14 15/mcmc_result/China_subnational_Beijing_mcmc_res.csv');

mcmcRes = mcmcRes(mcmcRes(:,1)~=0,:);
if size(mcmcRes,2) ~= 28
    error('Expected 28 Beijing parameters in the MCMC chain.');
end
...
mcLinDispMonthlyLab = zeros(revNumIterations,1);
...
for iiIter = 1:revNumIterations
    ...
    mcLinDispMonthlyLab(iiIter) = iterOutput.linDispMonthlyLab;
...
save('RSV_simulation_raw_results.mat', ...
    ...
    'mcEpsilonByAge','mcLinDispMonthlyLab','yearlyAgeSpecificIAR','populationAgeGroup', ...
...
for iiMonth = 1:length(mcRecMonthly(:,1))
    drawIdx = randsample(revNumIterations,numRnd,true);
    tempMu = max(reshape(mcRecMonthly(iiMonth,drawIdx),1,[]),1e-6);
    tempDisp = max(reshape(mcLinDispMonthlyLab(drawIdx),1,[]),1e-12);
    tempSize = max(tempMu./tempDisp,1e-12);
    tempP = tempSize./(tempSize + tempMu);
    mcRecMonthlyRnd(iiMonth,:) = nbinrnd(tempSize,tempP);
end
...
for iiMonth = 1:length(mcRecMonthlyByAge(:,1,1))
    temp1 = reshape(mcRecMonthlyByAge(iiMonth,:,:),[length(mcRecMonthlyByAge(1,:,1)),revNumIterations]);
    temp = (temp1(:,randsample(1:size(temp1,2),numRnd,true)))';
    temp2 = mnrnd(round(sum(temp,2)),temp./repmat(sum(temp,2),1,length(mcRecMonthlyByAge(1,:,1))));
