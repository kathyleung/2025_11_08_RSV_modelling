# Intervention Simulations — Thailand

**Folder:** `a3. RSV simulation only with mAb and vaccination Thailand/`

This folder simulates the population-level impact of RSV intervention strategies in Thailand using the calibrated SEIR model. Three intervention types are modelled:
...
→ Run `main_Thailand.m` in `7. RSV c mex Thailand R1 block update logL component/`  
  → Produces `mcmc_result/thailand_mcmc_res.csv`
...
| File | Description |
|------|-------------|
| `main_TL_result.m` | **Main simulation script** — runs intervention scenarios, saves `RSV_simulation.mat` |
| `main_TL_result_plot_impact.m` | Generates intervention impact figures |
...
| `age_SEIR_RSV_mex.mexmaca64` | Pre-compiled baseline MEX file (macOS Apple Silicon) |
