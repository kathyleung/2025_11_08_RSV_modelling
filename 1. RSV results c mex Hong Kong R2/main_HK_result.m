...
burnInStartIdx = max(1,floor(size(mcmcRes,1)*0.1)+1);
...
mcmcLogL = readmatrix('../1. RSV c mex Hong Kong R2 rev block update logL component rev NB fix reparam 14 15/mcmc_result/hongkong_log_likelihood_component.csv');
...
mcLinDispWeeklyLab = zeros(revNumIterations,1);
...
for iiIter = 1:revNumIterations
    ...
    mcLinDispWeeklyLab(iiIter) = x0(26);
...
save('RSV_simulation_raw_results.mat', ...
    ...
    'mcEpsilonByAge','mcLinDispWeeklyLab','yearlyAgeSpecificIAR','populationAgeGroup',...
...
for iiWeek = 1:length(mcRecWeekly(:,1))
    ...
    tempMu = max(reshape(mcRecWeekly(iiWeek,drawIdx),1,[]),1e-6);
    tempDisp = max(reshape(mcLinDispWeeklyLab(drawIdx),1,[]),1e-12);
    tempSize = max(tempMu./tempDisp,1e-12);
    tempP = tempSize./(tempSize + tempMu);
    mcRecWeeklyRnd(iiWeek,:) = nbinrnd(tempSize,tempP);
end
