...
% 1. MCMC results
mcmcResHK = readmatrix('../1. RSV c mex Hong Kong R2 rev block update logL component rev NB fix reparam 14 15/mcmc_result/hongkong_mcmc_res.csv');
mcmcResBJ = readmatrix('../4. RSV c mex Beijing R2 block update logL component NB fix reparam 14 15/mcmc_result/China_subnational_Beijing_mcmc_res.csv');
mcmcResTL = readmatrix('../7. RSV c mex Thailand R2 block update logL component NB fix reparam 14 15/mcmc_result/thailand_mcmc_res.csv');
...
epsilonByAge = zeros(length(mcmcRes(:,1)),numAgeGroups);
...
hold on
plot(ageGroupLower(1:13), prctileEpsilonHK(1,1:13), '-', ...
...
hold on
plot(ageGroupUpper(12:end), prctileEpsilonHK(1,12:end), '-', ...
...
fill(x_fill, y_fill, [0, 0.4470, 0.7410], 'FaceAlpha', 0.3, 'EdgeColor', 'none');
hold on
plot(ageGroupLower(1:13), prctileEpsilonBJ(1,1:13), '-', ...
    'Color', [0, 0.4470, 0.7410], 'LineWidth', 1);
xlim([0,1])
% ylim([0,0.2])
set(gca,'XTick',(0:2:12)/12,'XTickLabel',{'0m','2m','4m','6m','8m','10m','12m'})
...
fill(x_fill, y_fill, [0.8500, 0.3250, 0.0980], 'FaceAlpha', 0.3, 'EdgeColor', 'none');
hold on
plot(ageGroupUpper(12:end), prctileEpsilonBJ(1,12:end), '-', ...
    'Color', [0.8500, 0.3250, 0.0980], 'LineWidth', 1);
xlim([1,85])
% ylim([0,0.4])
set(gca,'XTick',[1,5:10:100])
...
fill(x_fill, y_fill, [0, 0.4470, 0.7410], 'FaceAlpha', 0.3, 'EdgeColor', 'none');
hold on
plot(ageGroupLower(1:13), prctileEpsilonTL(1,1:13), '-', ...
    'Color', [0, 0.4470, 0.7410], 'LineWidth', 1);
xlim([0,1])
% ylim([0,0.005])
set(gca,'XTick',(0:2:12)/12,'XTickLabel',{'0m','2m','4m','6m','8m','10m','12m'})
...
fill(x_fill, y_fill, [0.8500, 0.3250, 0.0980], 'FaceAlpha', 0.3, 'EdgeColor', 'none');
hold on
plot(ageGroupUpper(12:end), prctileEpsilonTL(1,12:end), '-', ...
    'Color', [0.8500, 0.3250, 0.0980], 'LineWidth', 1);
xlim([1,85])
% ylim([0,0.4])
set(gca,'XTick',[1,5:10:100])
